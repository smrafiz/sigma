...
class Breadcrumbs {
	...
	public function getBreadcrumbs( array $args ) {
		...
		$defaults = [
			...
			'displayTerms'           => true,
			'homeText'               => sprintf( '<i class="fa fa-home"></i> <span>%s</span>', esc_attr__( 'Home', 'signature' ) ),
			'catArchivePrefix'       => esc_attr__( 'Category:&nbsp;', 'signature' ),
			'tagArchivePrefix'       => esc_attr__( 'Tag:&nbsp;', 'signature' ),
			'searchPrefix'           => esc_attr__( 'Search query for:&nbsp;', 'signature' ),
			'errorPrefix'            => esc_attr__( '404 - Page not found', 'signature' ),
		];

		$defaults = apply_filters( 'sd/signature/breadcrumbs_defaults', $defaults );
	...
	private function getBreadcrumbTrailMarkup( $objectType = '' ) {
		...
		switch ( $objectType ) {
			...
			case 'year':
				...
				$title = esc_html__( 'Year: ', 'signature' ) . $year;
				break;
			...
			case 'month':
				...
				$title = esc_html__( 'Month: ', 'signature' ) . $wp_locale->get_month( $monthNum );
				break;
			...
			case 'day':
				...
				$title = esc_html__( 'Day: ', 'signature' ) . $day;
				break;
			...
			case 'author':
				...
				$title = esc_html__( 'Articles Posted by&nbsp;', 'signature' ) . $user->display_name;
				break;
