...
class Templates {
	...
	public function get( $slug, $name = null, $args = [], $load = true ) {
		...
		do_action( 'sd/signature/get_template_part_' . $slug, $slug, $name, $args );
	...
	protected function getFileNames( $slug, $name, $args ) {
		 ...
		 * @since 1.0.0
		 */
		return apply_filters( 'sd/signature/get_template_part', $templates, $slug, $name, $args );
	}
	...
	protected function getPaths() {
		$themeDirectory = trailingslashit( sd_signature()->getData()['ext_template_folder'] );

		$filePaths = [
			10  => trailingslashit( get_template_directory() ) . $themeDirectory,
			100 => sd_signature()->templatesPath(),
		];
		 ...
		 * @since 1.0.0
		 */
		$filePaths = apply_filters( 'sd/signature/template_paths', $filePaths );
