...
class Helpers {
	...
	public static function nonceText() {
		return 'sd_signature_nonce_secret';
	}
	...
	public static function verifyAjaxCall() {
		...
		if ( ! check_ajax_referer( self::nonceText(), self::nonceId(), false ) ) {
			wp_send_json(
				[
					'error'        => true,
					'errorMessage' => esc_html__( 'Security Check Failed. Access Denied!', 'signature' ),
				]
			);
		}
	}
	...
	public static function renderView( $viewName, $args = [] ) {
		...
		$file       = ltrim( $file, '/' );
		$pluginPath = sd_signature()->getThemePath();
		$viewsPath  = sd_signature()->getData()['views_folder'];
		...
		if ( ! file_exists( $viewFile ) ) {
			return new WP_Error(
				...
				sprintf( esc_html__( '%s file not found', 'signature' ), $viewFile )
			);
		}
	...
	public static function navMenuArgs( $args = [] ) {
		...
		$defaults = apply_filters( 'sd/signature/nav_menu_defaults', $defaults, $args );
