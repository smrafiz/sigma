...
namespace ThemeStarter\Signature\Common\Functions;
...
class Functions extends Base {
	/**
	 * Get theme data by using sd_signature()->getData()
	 *
	 * @return array
	 * @since 1.0.0
	 */
	...
	public function entryFooter() {
		...
		if ( 'post' === get_post_type() ) {
			...
			$categoriesList = get_the_category_list( esc_html__( ', ', 'signature' ) );
			if ( $categoriesList ) {
				/* translators: 1: list of categories. */
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'signature' ) . '</span>', $categoriesList ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}

			/* translators: used between list items, there is a space after the comma */
			$tagsList = get_the_tag_list( '', esc_html_x( ', ', 'list item separator', 'signature' ) );

			if ( $tagsList ) {
				/* translators: 1: list of tags. */
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'signature' ) . '</span>', $tagsList ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
