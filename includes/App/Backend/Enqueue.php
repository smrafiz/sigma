...
class Enqueue extends EnqueueBase {
	...
	protected function getStyles() {
		...
		$styles[] = [
			'handle'    => 'sd-signature-admin-styles',
			'asset_uri' => esc_url( $this->theme->assetsUri() . '/css/backend' . $this->suffix . '.css' ),
			'version'   => $this->theme->version(),
		];

		$this->enqueues['style'] = apply_filters( 'sd_signature_registered_admin_styles', $styles, 10, 1 );
	...
	protected function getScripts() {
		...
		$scripts[] = [
			'handle'     => 'sd-signature-admin-script',
		...
		$this->enqueues['script'] = apply_filters( 'sd_signature_registered_admin_scripts', $scripts, 10, 1 );
