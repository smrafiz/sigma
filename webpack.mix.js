...
if (
	...
	if (mix.inProduction()) {
		...
		fs.ensureDir(languages, function (err) {
			...
			wpPot({
				package: "Signature - A light-weight and simple WordPress theme boilerplate using an Object Oriented approach.",
				bugReport: "https://github.com/smrafiz/signature/issues",
				src: "**/*.php",
				domain: "signature",
				destFile: "languages/signature.pot",
			});
		});
	}
	 ...
	 */
	if (!mix.inProduction()) {
		mix.sass("src/sass/frontend.scss", "assets/css/frontend.min.css",).sourceMaps(true, 'source-map');
	} else {
		mix.sass("src/sass/frontend.scss", "assets/css/frontend.min.css");
	}
}
